...
/* 速度・トルク制御 モーション処理 */
static	BOOL MtgVelctrlWithAccr( MG_HNDL *hdl, MG_INOUT *inOut );
/* INTERPOLATEモーション処理 */
...
/* SPOSINGモーション処理 */
extern	BOOL MtgSposing( MG_HNDL *hdl, MG_INOUT *inOut );
/* SHOLDモーション処理 */
extern BOOL MtgShold( MG_HNDL *hdl, MG_INOUT *inOut );
/* S字減速距離計算 */
extern BOOL mtgSpCalDecPrm(MG_SPOSING* sposing, MC_BASE_PRM* bPrm, BOOL ChkReverse,
...
const MTGFUNC mtgOutputMotion[] = {
	...
	MtgVelctrl,					/* VELCTRL */
	MtgVelctrlWithAccr,			/* VELCTRL with Accr */
	MtgVelctrl,					/* TRQCTRL */
	MtgSposing,					/* SPOSING */
	MtgShold,					/* SHOLD */	
};
...
//	sd_dist0 = 0;
	...
	cspd = MlibLABS( spd );
	/* 減速パラメータセット */
	mtgSpSetPrm( &hdl->sposing, prm, 0);
	/* 減速距離計算			*/
	mtgSpCalDecPrm( &hdl->sposing, hdl->basePrm, TRUE, cspd, &sd_dist, &sd_dist0 );

	dist = sd_dist;              	                    /* buf = sd_dist (整数部) */
	if( ((sd_dist | sd_dist0) != 0) && ((sd_dist0 | hdl->dpos0) != 0) )
	{
		dist = dist + 1;								/* 減速距離小数部を繰り上げる */
	}
...
static	BOOL MtgVelctrl( MG_HNDL *hdl, MG_INOUT *inOut )
{
	...
	/*----------------------------------------------*/
#if (FLOAT_USE==TRUE)
	inOut->VrefOut = (LONG)( (float)inOut->Vref * hdl->basePrm->maxToOsKx );
#else
...
/****************************************************************************
  ...
  戻り値	:速度指令[2^24/OS速度]
****************************************************************************/
static LONG mtgMakeCrtVref( MG_HNDL *hdl, MG_INOUT *inOut )
{
	LONG SpdRef;
	LONG SpdRefx;
	KSGAIN kx;

	SpdRefx = inOut->VrefOut;
	kx = hdl->basePrm->m3ConvUnit.m3AccUnitToOs;

/*--------------------------------------------------------------------------*/
/*	前回制御モードが速度制御モードでない場合(制御モード切替の場合)			*/
/*--------------------------------------------------------------------------*/
	if( inOut->CtrlModeLst != MT_VEL_CTRLMD )
	{
		/*------------------------------------------------------------------*/
		/*	FB速度を速度目標値に設定										*/
		/*------------------------------------------------------------------*/
		SpdRef = inOut->FbSpeed;
		hdl->VrefSum[0] = 0;
		hdl->VrefSum[1] = SpdRef << 1;
...
/*--------------------------------------------------------------------------*/
/*		速度目標値 [2^30/MAX速度]-->[2^24/OS速度]							*/
/*--------------------------------------------------------------------------*/
#if (FLOAT_USE==TRUE)
	SpdRef = (LONG)( (float)inOut->Vref * hdl->basePrm->maxToOsKx );
#else
	SpdRef = MlibMulgain( inOut->Vref, hdl->basePrm->maxToOsKx );
#endif /* FLOAT_USE */
/*--------------------------------------------------------------------------*/
/*		ソフトスタート演算													*/
/*--------------------------------------------------------------------------*/
	if( SpdRef > SpdRefx )
	{
		if( SpdRefx >= 0 )
		{
			SpdRefx = MlibIntegral( hdl->prm->accr_vel, kx, hdl->VrefSum );
		}
		else
		{
			SpdRefx = MlibIntegral( hdl->prm->decr_vel, kx, hdl->VrefSum );
			if((hdl->prm->accr_vel != hdl->prm->decr_vel) && (SpdRefx >= 0))
			{
				SpdRefx = 0;
				hdl->VrefSum[0] = 0;
				hdl->VrefSum[1] = 0;
...
static	BOOL MtgVelctrlWithAccr( MG_HNDL *hdl, MG_INOUT *inOut )
{
	...
	/* 				速度指令作成					*/
	/*----------------------------------------------*/
	inOut->VrefOut = mtgMakeCrtVref( hdl, inOut );
...
#if (FLOAT_USE==TRUE)
	inOut->Tspd = (LONG)( (float)cSpd * hdl->basePrm->PosingSpdInvKx );
	inOut->VrefOut = (LONG)( (float)inOut->Vref * hdl->basePrm->refToOsKx );
#else
...
#if (FLOAT_USE==TRUE)
	tmp = (LONG)( (float)inOut->Tspd * hdl->basePrm->PosingSpdKx );
#else	
...
/*		速度指令積分処理(リミット：±2^30)									*/
/*																			*/
/*  dVref      : 加速度(速度増分値) [2^30/MAX速度]							*/
/*  VrefSum[1] : 速度指令積算値     [2^30/MAX速度]							*/
/*  VrefSum[0] : 速度指令演算時の余り										*/
/*  Gain       : {kx,sx}形式		sx=30以下とすること						*/
/*																			*/
/****************************************************************************/
VOID mtgVrefIntegral(LONG dVref, LONG Gain, LONG VrefSum[2])
{
LONG	x;
LONG	xx[2];
LONG	carry;
LONG	kx = ((Gain<<8)>>6);						/* Gainを4倍しておく	*/
LONG	sx = (((ULONG)Gain)>>24);

/*--------------------------------------------------------------------------*/
/*		加速度[ref/s^2]×ゲイン＝通信周期の速度指令増分値					*/
/*--------------------------------------------------------------------------*/
		MlibMul3232( dVref, kx, xx );
//		x = (xx[1] >>(sx+2));
		x =((xx[1] >> sx) >> 2);
/*--------------------------------------------------------------------------*/
/*		速度指令増分値の積算処理											*/
/*--------------------------------------------------------------------------*/
		if( x > 0 )
		{
			VrefSum[1] = VrefSum[1] + 0x3FFFFFFF;
		}
		else if( x < -1 )
		{
			VrefSum[1] = VrefSum[1] - 0x3FFFFFFF;
		...
		{
			carry = ( (ULONG)(VrefSum[0]+(xx[0]<<(30-sx))) < (ULONG)VrefSum[0] );
			VrefSum[0] = VrefSum[0] + (xx[0]<<(30-sx));
			VrefSum[1] = VrefSum[1] + (xx[1]<<(30-sx)) + ((ULONG)xx[0]>>sx>>2) + carry;
		}
/*--------------------------------------------------------------------------*/
/*		リミット処理														*/
/*--------------------------------------------------------------------------*/
		if( VrefSum[1] >= 0x40000000 )
		{
			VrefSum[0] = 0;
			VrefSum[1] = 0x40000000;
		}
		else if( VrefSum[1] < -0x40000000 )
		{
			VrefSum[0] =  0;
			VrefSum[1] = -0x40000000;
		}
