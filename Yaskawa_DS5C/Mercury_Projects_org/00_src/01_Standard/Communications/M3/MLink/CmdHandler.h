...
typedef struct CMD_INFO_ {
	...
	LONG			profileType;			/* プロファイルタイプ */
	BOOL			busy;					/* コマンド実行中フラグ(コマンド干渉チェック用) */
	BOOL			enableCmdAna;			/* コマンド解析実行要求フラグ */
	BOOL			cmdRdy;					/* コマンドのコマンドレディビット */
	BOOL			warng;					/* ワーニングビット */
	/* M3Stdプロファイル用 */
	ULONG			lstSvCmdCtrl;			/* SVCMD_CTRL前回値 */
	INT32			ResultCmdAna;			/* コマンド解析実行結果	*/
	LONG			lstCmdCtrl;				/* CMD_CTRL前回値 */	
	LONG			homingType;				/* 原点復帰タイプ */	
	LONG			ltSelR[2];				/* ラッチ信号選択情報 */
	BOOL			cmdPauseEnable;			/* CMD_PAUSE 有効フラグ */
	BOOL			cmdCancelEnable;		/* CMD_CANCEL 有効フラグ */
	BOOL			almClrCmp;				/* アラームクリア完了状態 1:アラームクリア完了 0:左記以外の状態 */
	BOOL			ltSeqEnable;			/* 連続ラッチ有効フラグ */
	BOOL			CmdAlm;					/* CMD_ALM状態フラグ */
	BOOL			reLatchReq;				/* ラッチ再要求フラグ */
