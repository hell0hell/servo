...
/* <S0FA> Start */
/*--------------------------------------------------------------------------------------------------*/
	/* Search Register (Monitor Register -Command Torque- => 42H：0x8001C004/40H：0xE002 */
	if ( ResFlag == TRUE )
	{
...
/* <S0FA> End */
/*--------------------------------------------------------------------------------------------------------------*/
/* Set Analog Torque Reference Offset Manual Adjust Mode (Fn00B) to Mode Register(42H：0x80004000/40H：0x2000)	*/
/*--------------------------------------------------------------------------------------------------------------*/
	if ( ResFlag == TRUE )
	{
...
/* <S0FA> Start */
	/* モニタレジスタ -内部トルク/推力指令(Un002)- (42H：0x8001C004/40H：0xE002) 読出し */
...
//				pv->var.DispAdiData --;				・/* <S142> */
			...
			else if ( OpKeyMake.data == TRUE )
			{
				...
				/* PnE56 : トルク指令ゼロ調(正入力/負入力)をEepRomへ書き込み */
